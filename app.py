...
# # Missing flags = 0 (user provided all values)
...
# # ----------------------------
# # SHAP (Option B: model-agnostic, single-patient, stable)
# # ----------------------------
# st.subheader("🧠 Explainability (SHAP) — Heart Disease")

# with st.expander("Show explanation"):
#     # Model-agnostic explainer (stable in your environment)
#     explainer = shap.Explainer(
#         lambda X: A["heart_model"].predict_proba(pd.DataFrame(X, columns=A["heart_feats"]))[:,1],
#         Xh
#     )
#     sv = explainer(Xh)
